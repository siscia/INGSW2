...
Taking forward the considerations made in the RASD, we analyse the interface between the world and the machine: given the list of goals and requirements, we identify what interfaces the system need to provide to user.
If those interfaces would have been alreay implemented works our work would be complete, however such is not the case, and those interface to work correctly require some functionalities.

Once all the functionalities have been identified we organize them into high-level components taking care to respect the ``Single Responsability'' principle in order to provide highly decoupled and reusable components.
