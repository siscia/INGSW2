...
Our risk mitigation strategy requires us to validate the product as early as possible. This request dictates slower development cycles, but we can be more sure about the final result are getting, which is very relevant also from a bussiness perspective. 
...
We start developing a very simple MVP where selected alpha users can rent cars once a day for a small nominal fee. 

A careful selection of the alpha tester should prevent the cars from being damaged, and prevent abuses on the alpha version of the system. At the same time, this process provides us extremelly valuable feedback on the system we are building and the interaction with the users.
...
After this phase, if the stackholders of the project agree, we proceed with the creation of the real platform and we decide what components of the MVP to keep (if any). The experience gained building the MVP will be extremelly valuable also after this phase, will let us move very fast and with confidence. During this phase we develop also the unit and the integration test.
...
Using this approach the developing time will be a little longer, but we are keeping the risk as low as possible. We can afford this since our design is extremely decoupled and simple to build, so our developing timings are short.
...
In the above Gantt chart we did not actually specified what time slot is associated with which activity. We simply gave an estimate of the time percentage the development activity should take on the whole time period.
...
Using these methodologies lets us incorporate the feedback from both the user and the test environment as soon as possible during th development cycle. 

For example, we may discover that the user is eager to pay something to keep the car booked for more time than a single hour: using our methodology we will be able to incorporate such feedback immediately in the system.
Moreover, if we discover that some of our modules is not able to keep up with the expected load, we could immediately allocate more resources and time to fix that side of the project.
