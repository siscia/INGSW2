...
Unit tests should be used in order to guarantee the interface exposed and documented are respected.
...
We start integrating the three services that are logically the same but physically distributed. In this way we can reason about them as a simpler logical component instead of multiple distributed components.

\begin{enumerate}
	\item CAR\_MANAGER: this component need its own integration. It is distributed between the car system and the main server; it is also the only components that touches the hardware side of the project. In our opinion it should be the first component to be tested.
	...
	\item POSITION: this component need it own integration too. However, its functionality is pretty standard and well know.
\end{enumerate}
...
Of course, as soon as a component is ready and integrated, it will be integrated with the User Interface.
...
Of course all the components need to be integrated with the user interface. We are not showing all those integration that, however, are extremely simple but necessary. Since the NOTIFIER components is part of the user interface, we are not showing the integrations that concern this component.

Before going deeply inside the integration, we show what integrations are necessary in order to treat all the components as a single logical entity, instead of multiple, more complex physical entities.
...
The use of stubs makes definitely sense when we want to simulate the physical car: we cannot have a car to run physically all the integration tests.

Also, the developing time of the hardware are usualy longer than the developing time of software, and it is reasonable to use stubs to simulate the hardware of the car.

Other stubs may be required to develop the integration test and will be written when necessary.
